...
public class CSVRecordReader extends RecordReader<LongWritable, TextArrayWritable> {
    ...
    private CSVParser parser;    // 新建CSVParser实例，用来解析每一行CSV文件的每一行
    ...
    }   // 使用LineRecordReader来得到下一条记录（即下一行）。  // 如果到了分片（Input Split）的尾部，nextKeyValue将返回NULL

    @Override
    public boolean nextKeyValue() throws IOException, InterruptedException {
        if (lineReader.nextKeyValue()) {      //如果有新记录，则进行处理
            loadCSV();
            return true;
        } else {
            value = null;
    ...
    private void loadCSV() throws IOException {
        String line = lineReader.getCurrentValue().toString();    // 通过OpenCSV将解析每一行的各字段
